...
const SPOT_TYPES = ["Skatepark", "Baranda", "Borde", "Escaleras", "Rampa", "Piso Liso"];
...
const ImageCarousel = ({ images }) => {
  ...
  return (
    <div className="relative w-full h-full min-h-[250px] group/carousel overflow-hidden bg-black">
...
export default function App() {
  ...
  const [newSpot, setNewSpot] = useState({ title: '', city: 'Neuquén', type: 'Skatepark', description: '', images: ['', '', '', ''], lat: '', lng: '' });
  const [newVideo, setNewVideo] = useState({ title: '', youtubeUrl: '' });
  const [editProfile, setEditProfile] = useState({ name: '', city: 'Neuquén', instagram: '', whatsapp: '', bio: '', photoUrl: '' });
    ...
    const unsubAuth = onAuthStateChanged(auth, async (u) => {
      ...
      if (u) {
        ...
        if (snap.exists()) {
          ...
          const initialData = { name: u.displayName || 'Rider', city: 'Neuquén', uid: u.uid, photoUrl: u.photoURL || '' };
  ...
  const handleSearchAddress = async () => {
    ...
    try {
      ...
      } else { setSearchStatus('No se encontró'); }
  ...
  const handleAddSpot = async (e) => {
    ...
    try {
      ...
      setNewSpot({ title: '', city: 'Neuquén', type: 'Skatepark', description: '', images: ['', '', '', ''], lat: '', lng: '' });
        ...
        <div className="flex flex-col md:flex-row items-end justify-between gap-8 mb-16 border-l-4 border-red-600 pl-6">
          ...
          <div className="flex flex-col sm:flex-row gap-4 w-full md:w-auto">
            <select className="bg-zinc-900 p-4 rounded-xl text-xs font-black uppercase outline-none border border-zinc-800" value={cityFilter} onChange={e => setCityFilter(e.target.value)}>
              ...
              {PROVINCES_ARGENTINA.map(p => <option key={p} value={p}>{p}</option>)}
            </select>
                ...
                <div className="p-6 space-y-4">
                  <div className="flex justify-between items-start">
                    ...
                    <div className="flex gap-2">
                      ...
                      {isUserAdmin && <button onClick={(e) => { e.stopPropagation(); if(window.confirm('¿Borrar?')) deleteDoc(doc(db, 'spots', spot.id)) }} className="bg-red-900/40 p-2 rounded-lg hover:bg-red-600"><Trash2 size={14}/></button>}
                    </div>
                  </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
            {videos.map(vid => (
               <div key={vid.id} className="bg-zinc-950 border border-zinc-900 rounded-3xl overflow-hidden relative group">
                  {isUserAdmin && <button onClick={() => { if(window.confirm('¿Borrar?')) deleteDoc(doc(db, 'videos', vid.id)) }} className="absolute top-4 right-4 z-10 bg-red-600 p-2 rounded-lg hover:bg-red-700 opacity-0 group-hover:opacity-100 transition-opacity"><Trash2 size={16}/></button>}
          ...
          <div className="bg-zinc-950 border border-red-600/30 p-8 rounded-[2rem] w-full max-w-lg shadow-2xl my-8 relative">
            ...
            <form onSubmit={handleAddSpot} className="space-y-5">
              ...
              <div className="grid grid-cols-2 gap-4">
                <select className="bg-zinc-900 p-4 rounded-xl text-white border border-zinc-800" value={newSpot.city} onChange={e => setNewSpot({...newSpot, city: e.target.value})}>{PROVINCES_ARGENTINA.map(p => <option key={p} value={p}>{p}</option>)}</select>
                <select className="bg-zinc-900 p-4 rounded-xl text-white border border-zinc-800" value={newSpot.type} onChange={e => setNewSpot({...newSpot, type: e.target.value})}>{SPOT_TYPES.map(t => <option key={t} value={t}>{t}</option>)}</select>
              </div>
        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 backdrop-blur-md bg-black/95 overflow-y-auto">
          <div className="bg-zinc-950 border border-zinc-800 p-8 lg:p-10 rounded-[2rem] w-full max-w-lg shadow-2xl my-8 relative">
            ...
            <form onSubmit={handleSaveProfile} className="space-y-6">
              ...
              <select className="w-full bg-zinc-900 p-4 rounded-xl outline-none border border-zinc-800" value={editProfile.city} onChange={e => setEditProfile({...editProfile, city: e.target.value})}>{PROVINCES_ARGENTINA.map(p => <option key={p} value={p}>{p}</option>)}</select>
              <div className="grid grid-cols-2 gap-4">
                <input className="w-full bg-zinc-900 p-4 rounded-xl outline-none border border-zinc-800 text-xs" placeholder="Instagram" value={editProfile.instagram} onChange={e => setEditProfile({...editProfile, instagram: e.target.value})} />
